...
class BabiDataset(Dataset):
    def __init__(self, dataset_dir='../bAbI/tasks_1-20_v1-2/en-10k/', task_id=1, mode='train'):
        self.vocab_path = os.path.join(dataset_dir, 'babi{}_vocab.pkl'.format(task_id))
        self.mode = mode
        raw_train, raw_test = get_raw_babi(dataset_dir, task_id)
        self.QA = adict()
...
def get_raw_babi(dataset_dir='../bAbI/tasks_1-20_v1-2/en-10k/', taskid=1):
    paths = glob(os.path.join(dataset_dir, 'qa{}_*'.format(taskid)))
    assert len(paths) > 0
    for path in paths:
        if 'train' in path:
...
if __name__ == '__main__':
    dset_train = BabiDataset(dataset_dir='../bAbI/tasks_1-20_v1-2/en', task_id=1)
